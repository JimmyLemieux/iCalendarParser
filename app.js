...
app.get('/getConflictEvents', function(req, res) {
  ...
  connection.query("SELECT * from EVENT WHERE start_time IN (SELECT (start_time), COUNT(*) AS c FROM EVENT GROUP BY start_time having c > 1)", function(err, rows, result) {
    if(err) throw err;
    else {
      for(let row of rows) {
        console.log(row);
